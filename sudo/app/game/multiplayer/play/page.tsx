...
function NameInputContent() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Navigate to room creation/join page (will implement later)
    router.push(`/game/multiplayer/room?difficulty=${difficulty}&name=${encodeURIComponent(name.trim())}`);
  };
        ...
        <div className="max-w-2xl rounded-3xl border-4 border-orange-400/60 bg-gradient-to-br from-orange-50/95 via-amber-50/95 to-orange-100/95 p-8 shadow-2xl backdrop-blur-sm sm:p-12 md:p-16">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <p className="mt-2 text-sm text-orange-600">
                This will be displayed during the match
              </p>
            </div>
          ...
          <div className="mt-6 rounded-xl bg-orange-100/60 p-4 text-center">
            <p className="text-sm font-medium text-orange-700">
              💡 Next: Create or join a room to compete
            </p>
          </div>
        </div>
...
export default function MultiplayerNameInputPage() {
  return (
